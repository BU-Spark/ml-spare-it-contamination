...
The notebook in this directory transforms the annotations of the TACO dataset(https://github.com/pedropro/TACO) into the format that Spare-it uses. As of now, the transformed data matches the existing Spare-it data exactly in terms of format of annotations(COCOJSON), but still uses their original (TACO) labels. For the POC stage, we will map the categories in the TACO dataset into the categories defined by Spare-it. For reference, listed below are categories in the TACO dataset.
